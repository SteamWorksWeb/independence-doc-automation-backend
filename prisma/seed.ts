...
async function main(): Promise<void> {
  ...
  const johnSnapshot = await prisma.dischargeSnapshot.create({
    data: {
      ...
      schoolClosed:           false,
      is65OrOlder:            false,
      lastAttendedSchool:     new Date("2015-05-01T00:00:00.000Z"),
      appliedForIDR:          true,
      madePriorPayments:      true,
      contactedServicer:      true,
  ...
  const janeSnapshot = await prisma.dischargeSnapshot.create({
    data: {
      ...
      rentExpense:            950,
      medicalExpense:         100,
      utilitiesExpense:       180,
      homeMaintenanceExpense: 0,
      carInsuranceExpense:    120,
      gasExpense:             80,
      isEmployed:             false,
      workInFieldOfStudy:     false,
